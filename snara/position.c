...
// unused
static bool alignment(t_bitmap pos)
{
	t_bitmap m;
	// horizontal
	m = pos & (pos >> (HEIGHT+1));
...
static t_bitmap	row_mask(int row)
{
...
static t_bitmap opponent_winning_position(const t_board *self)
{
...
static bool is_winning_move(const t_board *self, int col)
{
...
size_t play_seq(t_board *self, const char *seq)
{
	...
	{
		int col = seq[i] - '1';
		if (col < 0 || col >= WIDTH || !can_play(self, col))
